...
void uchar_transpose_avx(__m256i *src, __m256i *dst, int n)
{
	...
	while (loop--) {
		...
		copy_p_input += constN;
		__m256i c = LOAD_SIMD_FX(copy_p_input);
		copy_p_input += constN;
...
void ReadProfile(Parameter_Simulation * p)
{
	...
	fin >> rub >> p->snr_pass;
	fin >> rub >> p->snr_end;
	fin >> rub >> p->decode_method;
	fin >> rub >> p->Max_Iteration;
	fin >> rub >> rub;
	fin >> rub >> p->mod_type;
	fin >> rub >> p->interleavemod_type;
	fin >> rub >> rub;
	fin >> rub >> p->Factor_1;
	fin >> rub >> p->Factor_2;
	fin >> rub >> p->nb_frames;
	fin >> rub >> p->scale;
	fin >> rub >> rub;
	p->fileName = (char *)malloc(80 * sizeof(char));
	fin >> rub >> p->fileName;
	p->fileName = MATRIX_FILE;
	fin >> rub >> p->Z;
